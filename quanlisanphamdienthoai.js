...
function searchIphone() {
    ...
    for (let i = 0; i < arrIphone.length; i++) {
        if (inputIphone.value === arrIphone[i].nameIphone) {
            document.getElementById("showIphone").innerHTML = arrIphone[i].nameIphone +`<br><button type="button" onclick="information(${i})">Information</button>&nbsp;&nbsp;&nbsp;&nbsp;         <button type='button' onclick='del(${i})'>Delete</button>&nbsp;&nbsp;&nbsp;&nbsp;<button>Edit</button><br><hr>`;
            break;
        } else {
...
function information(index) {
    document.getElementById("showIphone").innerHTML = arrIphone[index].thongTinIphone() +`<br><button type='button' onclick='del(${index})'>Delete</button>&nbsp;&nbsp;&nbsp;&nbsp;<button>Edit</button><br><hr>`;
}