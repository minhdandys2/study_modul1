...
function searchIphone() {
    ...
    for (let i = 0; i < arrIphone.length; i++) {
        if (inputIphone.value === arrIphone[i].nameIphone) {
            document.getElementById("showIphone").innerHTML = arrIphone[i].nameIphone +`<br><button type="button" onclick="information(${i})">Information</button>&nbsp;&nbsp;&nbsp;&nbsp;         <button type='button' onclick='del(${i})'>Delete</button>&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" onclick="edit(${i})">Edit</button><br><hr>`;
            break;
        } else {
